...
describe('privacy and adapters', () => {
  ...
  it('Gemini sends official roles/header/system instruction and normalizes usage', async () => {
    ...
    const response = await new GeminiProvider().generate([{role:'system',content:'rules'},...messages,{role:'assistant',content:'prior'}],candidate('gemini',{model:'gemini-3.7-flash'}),new AbortController().signal,{})
    ...
    expect(body.generationConfig).toMatchObject({maxOutputTokens:2048,thinkingConfig:{thinkingLevel:'low'}})
    expect(body.generationConfig).not.toHaveProperty('temperature')
  })
  it('Gemini never returns a visibly truncated MAX_TOKENS answer', async () => {
    vi.stubEnv('GEMINI_API_KEY','test-only'); vi.stubEnv('AI_ALLOW_EXTERNAL_PROVIDERS','true')
    vi.stubGlobal('fetch',vi.fn().mockResolvedValue(new Response(JSON.stringify({candidates:[{finishReason:'MAX_TOKENS',content:{parts:[{text:'Only an intro:'}]}}]}))))
    await expect(new GeminiProvider().generate(messages,candidate('gemini',{model:'gemini-3.7-flash'}),new AbortController().signal,{maxOutputTokens:450}))
      .rejects.toMatchObject({code:'TRUNCATED_RESPONSE',message:'TRUNCATED_RESPONSE'})
  })
  it('classifies Gemini model/config 400 responses as configuration failures', async () => {
    vi.stubEnv('GEMINI_API_KEY','test-only'); vi.stubEnv('AI_ALLOW_EXTERNAL_PROVIDERS','true')
    vi.stubGlobal('fetch',vi.fn().mockResolvedValue(new Response(JSON.stringify({error:{message:'private upstream body'}}),{status:400})))
    await expect(new GeminiProvider().generate(messages,candidate('gemini',{model:'gemini-3.7-flash'}),new AbortController().signal,{}))
      .rejects.toMatchObject({code:'CONFIG',status:400,message:'CONFIG'})
  })
