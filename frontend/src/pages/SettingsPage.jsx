      ...
      <div className={styles.settingsShell}>
        ...
        <div className={styles.flow}>
          ...
          <SettingsSection id="eposta" icon={<Mail />} title="E-posta adresi" description="Giriş yaptığınız e-posta adresini mevcut şifrenizle güvenli biçimde değiştirin.">
            <form onSubmit={saveEmail}><Field label="Yeni e-posta"><input type="email" autoComplete="email" value={emailForm.email} onChange={event => setEmailForm(current => ({ ...current, email: event.target.value }))} required /></Field><Field label="Mevcut şifre" hint="Değişikliği doğrulamak için gereklidir."><input type="password" autoComplete="current-password" value={emailForm.password} onChange={event => setEmailForm(current => ({ ...current, password: event.target.value }))} required /></Field><Footer message={<Message msg={emailMsg} />}><Button type="submit" disabled={emailSaving}>{emailSaving ? 'Güncelleniyor…' : 'E-postayı değiştir'}</Button></Footer></form>
          </SettingsSection>
