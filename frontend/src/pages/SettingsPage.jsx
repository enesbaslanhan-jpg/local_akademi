      ...
      <div className={styles.settingsShell}>
        ...
        <div className={`${styles.flow} ${styles[`show_${activeSection}`]}`}>
          ...
          <SettingsSection id="guvenlik" icon={<ShieldCheck />} title={t('settings.securitySection.title')} description={t('settings.securitySection.description')}>
            {/* Sosyal girişle açılan hesapta mevcut parola YOK: değiştirme formu
                yerine sıfırlama akışıyla "parola belirle" (16.09.2026). */}
            {user?.hasPassword === false ? (
              <div className={styles.dangerNotice} style={{ background: 'transparent' }}>
                {t('settings.securitySection.noPassword')}{' '}
                <Link to="/forgot-password">{t('settings.securitySection.setPassword')}</Link>
              </div>
            ) : (
